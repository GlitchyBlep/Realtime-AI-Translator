"""
实时同传翻译器 - 完整修复版
功能：实时监听系统音频/麦克风 → 智能断句 → 流式翻译 → 左右分栏显示
使用方法：python pro_translator.py
"""
...
class SpeechRecognizer(QThread):
    ...
    def __init__(self, config: Config, audio_queue: queue.Queue):
        ...
        self.is_running = False
        self.language = None  # 初始化为None - 修复点1
        self.model = None
    ...
    def set_language(self, language: str):
        """设置识别语言 - 修复点1：处理auto为None"""
        if language == "auto" or language == "自动检测":
            self.language = None  # Whisper使用None表示自动检测
        else:
...
class StreamTranslator(QThread):
    ...
    def __init__(self, config: Config, text_queue: queue.Queue):
        ...
        self.target_lang = "zh"
        self.client = None
        self.translation_history = []  # 翻译历史记录
...
class FloatingWindow(QWidget):
    ...
    def update_translation_complete(self, translation: str):
        ...
        if lines and lines[-1].startswith("[翻译中]"):
            # 替换最后一行
            lines[-1] = f"[{timestamp}] {translation}"
        else:
            # 添加新行
...
class ControlWindow(QMainWindow):
    ...
    def init_ui(self):
        """初始化UI"""
        self.setWindowTitle("专业级同传翻译器 - 控制台")
    ...
    def on_language_changed(self):
        ...
        # 更新状态显示
        self.update_status(f"语言设置已更新: {source_lang} → {target_lang}")
...
def main():
    ...
    app.setApplicationName("专业级同传翻译器")
    app.setApplicationDisplayName("专业级同传翻译器")
