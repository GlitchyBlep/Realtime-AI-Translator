...
class StreamTranslator(QThread):
    ...
    def set_languages(self, source_lang: str, target_lang: str):
        ...
        self.source_lang_code = lang_map.get(source_lang, "auto")
        self.target_lang_code = lang_map.get(target_lang, "Chinese")
    ...
    def build_prompt(self, text: str) -> str:
        """构建翻译提示词 - 修复点4"""
        # 暴力中文指令
        return f"你是一个极其专业的同传翻译。请直接给出下面这段话的中文翻译，绝对不要重复原文，不要加任何标点和解释：\n\n原文：{text}\n\n译文："
            ...
            try:
                ...
                try:
                    ...
                    response = self.client.generate(
                        ...
                        stream=True,
                        options={
                            "temperature": 0.1  # 只保留temperature，移除num_predict
                        }
                    )
                    ...
                    print(f"❌ 翻译错误: {e}")
                    # 发送错误信息
                    self.translation_complete.emit(f"[翻译错误]")
...
class FloatingWindow(QWidget):
    ...
    def update_translation_chunk(self, chunk: str):
        ...
        if self.subtitle_lines:
            # 取最后一条原文
            last_original = self.subtitle_lines[-1]
